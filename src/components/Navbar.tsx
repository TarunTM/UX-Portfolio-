...
import { Home, Award, Wrench, Briefcase, Mail, Gamepad2, Sun, Moon } from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  const [theme, setTheme] = useState<'light' | 'dark'>(() => {
    ...
    // Default to light (warm editorial)
  ...
  useEffect(() => {
    ...
    const observerOptions = {
      root: null,
      rootMargin: '-20% 0px -50% 0px', // Center region of viewport
      threshold: 0.1,
    };
  ...
  const currentActive = isCaseStudy ? 'work' : activeSection;

  return (
    <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 lg:bottom-auto lg:top-1/2 lg:left-8 lg:-translate-y-1/2 lg:-translate-x-0 z-[1000] select-none">
      <div className="flex flex-row lg:flex-col items-center gap-2.5 px-3 py-2.5 lg:px-2.5 lg:py-3 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-xl border border-neutral-200/50 dark:border-zinc-800/80 rounded-full shadow-[0_12px_30px_rgba(0,0,0,0.06)] dark:shadow-[0_12px_45px_rgba(0,0,0,0.4)] transition-all duration-300">
        
        {/* Home Button */}
        <button
          onClick={() => handleNav('home')}
          className={`w-11 h-11 rounded-full flex items-center justify-center transition-all duration-300 active:scale-90 relative group cursor-pointer ${
            currentActive === 'home'
          ...
          aria-label="Home"
        >
          <Home className="w-4.5 h-4.5 stroke-[1.75]" />
          <span className="absolute bottom-14 lg:bottom-auto lg:left-14 lg:top-1/2 lg:-translate-y-1/2 px-2.5 py-1 rounded-md bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 text-[10px] font-mono opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap shadow-md border border-neutral-800 dark:border-neutral-200 z-50">
            Home
          </span>
        </button>
        ...
        >
          <Gamepad2 className="w-4.5 h-4.5 stroke-[1.75]" />
          <span className="absolute bottom-14 lg:bottom-auto lg:left-14 lg:top-1/2 lg:-translate-y-1/2 px-2.5 py-1 rounded-md bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 text-[10px] font-mono opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap shadow-md border border-neutral-800 dark:border-neutral-200 z-50">
            Minigame
          </span>
        </button>
        ...
        <div className="w-[1px] h-5 lg:w-5 lg:h-[1px] bg-neutral-200 dark:bg-zinc-800 mx-1 my-0.5" />

        {/* Theme Toggle Button */}
        <button
          onClick={toggleTheme}
          className="w-11 h-11 rounded-full flex items-center justify-center transition-all duration-300 active:scale-90 relative group cursor-pointer bg-neutral-100/70 text-neutral-500 hover:bg-neutral-200/80 hover:text-neutral-800 dark:bg-zinc-900/60 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-white"
          aria-label="Toggle Theme"
        >
          {theme === 'light' ? (
            <Moon className="w-4.5 h-4.5 text-accent stroke-[1.75]" />
          ) : (
            <Sun className="w-4.5 h-4.5 text-amber-400 stroke-[1.75]" />
          )}
          <span className="absolute bottom-14 lg:bottom-auto lg:left-14 lg:top-1/2 lg:-translate-y-1/2 px-2.5 py-1 rounded-md bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 text-[10px] font-mono opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap shadow-md border border-neutral-800 dark:border-neutral-200 z-50">
            Theme
          </span>
